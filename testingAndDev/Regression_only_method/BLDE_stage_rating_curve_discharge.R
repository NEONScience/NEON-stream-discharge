...
library(lubridate)
library(plotly)
surfElv<-loadByProduct(dpID="DP1.20016.001", site="BLDE", startdate="2018-10", enddate="2019-09", 
...
dscGagings<-loadByProduct(dpID="DP1.20048.001", site="BLDE", startdate="2018-10", enddate="2019-09", 
                       package="expanded", check.size = F)
...
figure<-figure%>%layout(title = "Stage Regression", xaxis = list(title = "Troll Elev (m)"),
                        yaxis = list (title = "Gage Height (m)"),annotations=list(text=eq,x=2023.8,y=0.5,showarrow=FALSE))
figure
...
fit<-nls(totalDischarge~c*streamStage^n, data=dscGagings,start=list(c=1000,n=4))
cf<-round(coef(fit),digits=8)
...
figure<-figure%>%layout(title = "Rating Curve", xaxis = list(title = "Stage (m)"),
                        yaxis = list (title = "Discharge (L/s)"),annotations=list(text=eq,x=0.4,y=2500,showarrow=FALSE))
figure
...
figure<-figure%>%layout(title = "Discharge Regression", xaxis = list(title = "Est Q (L/s)"),
                        yaxis = list (title = "Measured Q (L/s)"),annotations=list(text=eq,x=1000,y=2500,showarrow=FALSE))
figure

...
#' @param gaugingsData Data frame containing gaugings [dataframe]
...
BaM.run.pred.RC <- function(
  ...
  #Write out the gaugings file if needed -- If working with outputs of calc.stag.Q.curv there will already be gaugings written out from model, so this step is skipped
  if(!file.exists(paste0(DIRPATH, BAMWS, "data/Gaugings.txt"))){
    ...
    gaugings <- data.frame(matrix(data=NA, ncol=length(gagNam), nrow=length(gaugingsData$gaugeHeight)))
    names(gaugings) <- gagNam

    #Zeroes below assume no uncertainty, may want to change that
    gaugings$H <- gaugingsData$gaugeHeight #Stream stage values (m)
    gaugings$uH <- 0.00 #May include in the future
    gaugings$bH <- 0.00
    gaugings$bHindx <- 0.00
    gaugings$Q <- gaugingsData$streamDischarge #Stream discharge values (lps), re-calculated
    gaugings$uQ <- gaugingsData$streamDischargeUnc
    gaugings$bQ <- 0.00

...
surveyData1<-read.csv(paste0(BoxDir,"/L4-Discharge-Development-And-Testing/allSites/Discharge_XS/",siteID,"/",domainID,"_",siteID,"_surveyPts_",surveyDate1,"_both.csv"))
...
dischargePointsXS1<-subset(surveyData1,mapCode=="Transect_S2")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name%in%c("S2_XS1","S2_XS")]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name%in%c("S2_XS1","S2_XS")]
...
staffGauge1MeterMark<-as.numeric(gsub("M","",gsub("[A-Z]{2}_","",staffGaugePoints1$name)))
staffGauge1Elevation <- staffGaugePoints1$H

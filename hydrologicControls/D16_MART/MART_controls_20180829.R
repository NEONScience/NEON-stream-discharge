...
#' discharge rating curve for Martha Creek for water years 2012-2019.
...
filepath <- "N:/Science/AQU/Controls/D16_MART_20180829"
URIpath <- paste(filepath,"filesToStack20048","stackedFiles",sep = "/")
...
surveyPtsDF <- read.table(paste0(URIpath,"/D16_MART_surveyPts_20180829.CSV"),
                          sep = ",",
                          header = TRUE,
...
staffGaugePoints=subset(surveyPtsDF,surveyPtsDF$mapCode=="Gauge_Base")
staffGaugePoints<-staffGaugePoints[order(staffGaugePoints$N),]
rownames(staffGaugePoints)<-seq(length=nrow(staffGaugePoints))

#Set meter mark where the staff gauge was shot in and the name of the staff gauge point:   ##################################################################### Not in Survey Data ####################################################################
#Recorded in field data
staffGaugeMeterMark<-0.0
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_LFW"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC18"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC18"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_PIN_RB"]
...
geo_controlType_in$channelSlopeUnc[2] <- 0.014  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- rise/run
geo_controlType_in$channelSlopeUnc[3] <- 0.014  #Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_REW"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC19"]

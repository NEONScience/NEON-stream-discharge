...
#' discharge rating curve for Toolik Outlet for water years 2022-.
...
filepath <- "N:/Science/AQU/Controls/D18_TKOT_20220811"

surveyPtsDF <- read.table("~/D18_TKOT_surveyPts_20220811.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="OUT_DSC_XS41"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="OUT_DSC_XS41"]
...
staffGaugePoints=subset(surveyPtsDF,surveyPtsDF$name=="OUT_SP_0.60M")
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-10,30))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_MAX_LEW"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_MAX_LEW"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_XS41"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_MAX_LEW"]
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_XS12"]
geo_priorParameters_in$priorActivationStageUnc[1] <- 0.1

geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_XS26"]
geo_priorParameters_in$priorActivationStageUnc[2] <- 0.1

geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_MAX_LEW"]

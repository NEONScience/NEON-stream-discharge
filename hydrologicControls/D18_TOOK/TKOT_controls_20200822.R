...
#' discharge rating curve for Toolik Lake outlet for water years 2012-.
...
filepath <- "N:/Science/AQU/Controls/D18_TKOT_20200822"
...
L0pull_site <- restR::get.os.l0.by.namedLocation(
  pullType = "startDate",
  stack = "cert",
  tab = 'DP0.00132.001:bat_AISsiteSurveyResultsFile_in',
  minDate = '2020-08-01',
  maxDate = '2020-09-01',
  namedLocationName = 'TOOK')
L0pull_TKIN<-L0pull_site[(L0pull_site$rawDataFileName=="NEON_D18_TKOT_20200822_AIS_SURVEY_L0_VA.zip"),]  
download.file(L0pull_TKIN$rawDataFilePath,L0pull_TKIN$rawDataFileName,mode="wb")
unzip(paste0("~/",L0pull_TKIN$rawDataFileName),exdir="~")
surveyPtsDF <- read.table("~/D18_TKOT_surveyPts_20200822.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="OUT_DSC_46"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="OUT_DSC_46"]
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="OUT_DSC_46"]
...
floodPlain<-data.frame(name=c("OUT_DSC_FLD"),E=c(0.0),N=c(0.0),H=c(995.2),mapCode=c("Transect_DSC"),DistanceRaw=c(-15),DistanceAdj=c(-15))
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-11,24))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_44"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_5"]
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_37"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_25"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_RBF"]

...
#' discharge rating curve for Rio Guilarte for water years 2018.
...
filepath <- "N:/Science/AQU/Controls/D04_GUIL_20180321"
...
L0pull_site <- restR::get.os.l0.by.namedLocation(
  pullType = "startDate",
  stack = "cert",
  tab = 'DP0.00131.001:geo_resultsFile_in',
  minDate = '2018-01-01',
  maxDate = '2018-08-01',
...
staffGaugePoints=subset(surveyPtsDF,surveyPtsDF$name=="SP_1.05M_TEMP")
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "N_DSC4"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "N_DSC17"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "LBF_DSC_N"]

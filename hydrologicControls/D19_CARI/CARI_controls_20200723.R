...
#' discharge rating curve for Caribou Creek for water years 2020-
...
#   Bobby Hensley (11/11/2021)
#     Modified for CARI 2020 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D19_CARI_20200723"
...
surveyPtsDF <- read.table("~/D19_CARI_surveyPts_20200723.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
floodplain<-data.frame("DSC_FLP","18.5","26","999.4","Transect_DSC")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_XS2"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_XS2"]
...
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_0.50",staffGaugePoints$name)]  
...
#Slope calculations
# No THW or WE shots in survey. Used 2017 survey slope.
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS23"]

...
#' discharge rating curve for McDiffett Creek for water years 2012-2019
...
#   Bobby Hensley (5/17/2021)
#     Modified for MCDI 2018 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D06_MCDI_20181128"
...
surveyPtsDF <- read.table("~/D06_MCDI_surveyPts_20181128.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
surveyActiveDate <- "2012-01-01"    #First survey set to WY12 by default 
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_XS_LB_FLD4"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_XS_LB_FLD4"]
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_XS_LB_PIN"]
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-60,40))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_LEW"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_REW"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS8"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_RBF"]
...
geo_controlType_in$controlLeft[4] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_RBF"]
geo_controlType_in$controlRight[4] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_RB_FLD3"]
...
wettedEdgePoints=subset(surveyPtsDF,surveyPtsDF$mapCode%in%c("Slope_REW","Slope_LEW"))
...
geo_controlType_in$channelSlopeUnc[4] <- 0.005 # Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS24"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_LEW"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS6"]
...
geo_priorParameters_in$priorActivationStage[4] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_RB_FLD2"]

...
#' discharge rating curve for Rio Cupeyes for water years 2020-.
...
#   Bobby Hensley (8/25/2020)
#     Modified for CUPE 2020 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D04_CUPE_20200309_NEW"
...
surveyPtsDF <- read.table("~/D04_CUPE_surveyPts_20200309.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
surveyActiveDate <- "2020-03-09" #Need to update for each transect in this survey once Nick figures out which they correspond to

#Stipulate 4-digit site code, underscore, and survey year (ex: HOPB_2017)
surveyID <- "CUPE_2020"

#Creates dataframe of all points associated with transect DSC_NEW
names(surveyPtsDF) <- c("name","E","N","H","mapCode")
dischargePointsXS1<-subset(surveyPtsDF,mapCode=="Transect_DSC_NEW")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_NEW38"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_NEW38"]
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_NEW38"]
...
staffGaugePoints=subset(surveyPtsDF,surveyPtsDF$name=="SP_0.8M")
...
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_0.8M",staffGaugePoints$name)]  
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(0,25))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_NEW_LEW"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_NEW_REW"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_NEW_LFW"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_NEW_RFW"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_NEW_RFW"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_NEW"]
...
#Slope calculations - No water shots in survey. Used slope from 2018 Geomorph survey
geo_controlType_in$channelSlope[2] <- 0.022
geo_controlType_in$channelSlopeUnc[2] <- 0.022  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.022
geo_controlType_in$channelSlopeUnc[3] <- 0.022  #Default slope uncertainty is equal to slope

#chosen to represent stream conditions with higher roughness above bankfull
geo_controlType_in$manningCoefficient[2] <- 0.05 # Sluggish weedy reaches
...
geo_controlType_in$manningCoefficient[3] <- 0.3 # Grassland and scrub
geo_controlType_in$manningCoefficientUnc[3] <- 0.15 # Default Mannings uncertainty equal 50%
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_NEW28"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_NEW_LEW"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_NEW_RFW"]

...
filepath <- "N:/Science/AQU/Controls/D18_OKSR_20190911"
URIpath <- paste(filepath,"filesToStack00131","stackedFiles",sep = "/")

# OKSR survey points were not in API.  Pulled these from the N:/ drive
# N:\Special\AIS Survey Work\D18_OKSR\OKSR_20190911_QXS_TROLL_Gauge_Survey\Raw_Survey_Data



#Read in downloaded data
surveyPtsDF <- read.table(paste0(filepath,"/D18OKSR_surveyPts_20190911.CSV"),
                          sep = ",",
                          header = TRUE,
...
#Sets plot1 settings.  
xAxisTitle1<-list(title="Easting (m)",zeroline=FALSE, range=c(-325,-300))
...
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC19"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC33"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC24"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC24"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC37"]
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC4"]

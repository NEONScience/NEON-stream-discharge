######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D16 - MCRA
...
#' discharge rating curve for McRae Creek for water years 2022-
...
#   Bobby Hensley (2022-12-20)
#     Script for MCRA 2022-08-31 created.
...
dataFromAPI <- neonUtilities::loadByProduct(streamMorphoDPID,siteID,startdate="2022-08", enddate="2022-08",
...
#' There was also a rapid habitat done this month, so manually telling code to use AIS survey
...
#Creates dataframe of staff gauge points (This survey had an incorrect map code, so used surveyPointID)
staffGaugePoints=subset(geo_surveyPoints,geo_surveyPoints$surveyPointID=="SP_0.5m")
...
staffGaugeElevation <- staffGaugePoints$elevation[grepl("SP_0.5m",staffGaugePoints$surveyPointID)]  
...
geo_controlType_in$channelSlope[2] <- 0.09
geo_controlType_in$channelSlopeUnc[2] <- 0.09 #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.09
geo_controlType_in$channelSlopeUnc[3] <- 0.09 #Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[1] <- dsc_surveyPoints$gaugeHeight[dsc_surveyPoints$surveyPointID == "DSC_XS_15"]
geo_priorParameters_in$priorActivationStageUnc[1] <- 0.1
geo_priorParameters_in$priorActivationStage[2] <- dsc_surveyPoints$gaugeHeight[dsc_surveyPoints$surveyPointID == "DSC_XS_16"]
geo_priorParameters_in$priorActivationStageUnc[2] <- 0.1 
geo_priorParameters_in$priorActivationStage[3] <- dsc_surveyPoints$gaugeHeight[dsc_surveyPoints$surveyPointID == "DSC_XS_19"]

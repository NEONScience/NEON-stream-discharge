...
surveyPtsDF <- read.table(paste0(URIpath,"/D06_KING_surveyPts_20191219.csv"),
                          sep = ",",
                          header = TRUE,
...
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_0.9",staffGaugePoints$name)]  
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-20,50))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS38"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS21"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS78"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS10"]
...
wettedEdgePoints=subset(surveyPtsDF,surveyPtsDF$mapCode%in%c("Slope_Upstream","Slope_Downstream"))
...
geo_controlType_in$channelSlopeUnc[2] <- 0.02 # Slope was really small here. Used uncertainty from other KING survey
geo_controlType_in$channelSlope[3] <- rise/run
geo_controlType_in$channelSlopeUnc[3] <- 0.02 # Slope was really small here. Used uncertainty from other KING survey
